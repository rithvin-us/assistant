...
uniform vec2 iResolution; uniform float iTime; uniform float uVolume;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float low  = clamp(0.45 + 0.45*sin(t*0.45)*sin(t*0.25+1.0) + vol * 0.6, 0.0, 2.0);
    float mid  = clamp(0.40 + 0.40*sin(t*0.9+2.0)*sin(t*0.35) + vol * 0.7, 0.0, 2.0);
    float high = clamp(0.30 + 0.30*sin(t*1.4+4.0)*sin(t*0.4+2.0) + vol * 0.5, 0.0, 2.0);

    float res   = clamp(RESOLVED, 0.0, 1.0);
    float drift = mod(t, 20.0*PI) * (SPEED + vol * 0.45);

    float xN  = p.x / max(aspect, 1.0);
    float env = cos(PI*0.5 * min(abs(0.9*xN), 1.0));
    env *= env;

    float A1    = AMPLITUDE * (1.0 + vol * 0.9) + 0.01*low*LOW_AMP;
    float A2    = A1 + mid*MID_ABAMP + high*HIGH_ABAMP + vol * 0.08;
    float AB    = (ABERRATION + mid*MID_ABER + high*HIGH_ABER + vol * 1.4)*res;
    float th    = mix(0.1, 0.01*(THICKNESS + vol * 1.8), res);
    float inten = mix(0.1, 0.01*(INTENSITY + low*LOW_INT + vol * 2.6), res);
...
void main(){
    ...
    float aa = clamp((r - dist) + 0.5, 0.0, 1.0);
    gl_FragColor = vec4(gl_FragColor.rgb * aa, aa);
}`;
...
const int   N   = 6;
const float SMOOTH_K = 0.13;
const float INTENSITY  = 0.0045;
const float FALLOFF_P  = 1.35;
const float FADE_START = 0.03;
const float FADE_END   = 0.85;
const float ABERR = 0.008;
const vec3  SPECTRAL = vec3(0.0, 0.5, 1.0) * ABERR;
const float HUE_SPEED = 0.06;
const float COLOR_K   = 0.45;
...
float dotR(float fi, float seed, float t){
    return 0.078 + 0.016*sin(t*1.3 + seed*TAU) + 0.009*sin(t*2.4 + fi*1.3);
}
float dotSD(vec2 p, vec2 pos, float r, float t, float fi, float shapeDamp){
    vec2 d = p - pos;
    float sq = 0.085 * (0.5 + 0.5*sin(t*0.9 + fi*2.0)) * shapeDamp;
...
vec3 scene(vec2 p, float t){
    ...
    for(int i=0; i<N; i++){
        ...
        // Expanded orbit radius for much bigger circular choreography
        float R = 0.44 + 0.020*sin(t*1.0) + 0.015*sin(t*1.3 + seed*TAU);
...
function ShaderCanvas({
  ...
  React.useEffect(() => {
    ...
    const fs = compile(gl.FRAGMENT_SHADER, shaderSrc);
    ...
    const uVol = gl.getUniformLocation(program, "uVolume");
    ...
    const frame = () => {
      ...
      if (uVol) {
        gl.uniform1f(uVol, audioLevelRef.current);
      }
...
export interface SiriWaveProps
  ...
  renderScale?: number;
  /** Live audio volume level (0.0 to 1.0) to sync wave animation with voice. */
  audioLevel?: number;
}
...
export function SiriWave({
  ...
  return (
    ...
    >
      ...
      />
      <ShaderCanvas
        shaderSrc={FLUID_DOTS_SHADER}
        size={size}
        renderScale={renderScale}
        audioLevel={0}
        active={variant === "fluid-dots"}
      />
    </div>
  );
}
